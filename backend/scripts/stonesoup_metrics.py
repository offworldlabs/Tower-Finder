...
class MetricRecorder:
    ...
    def record_publish(
        self, t_s: float, key: str, lat: float, lon: float, vel_east_ms: float = 0.0, vel_north_ms: float = 0.0
    ) -> None:
        ...
        for key, samples in self._publishes.items():
            ...
            for grid_t in self._grid_times:
                ...
                if not covered:
                    ...
                    continue
                if current is None:
                    track_id = str(key) if next_segment_n == 0 else f"{key}#s{next_segment_n}"
    ...
    def compute(self) -> dict | None:
        ...
        gospa_metric = _find_metric_by_title(gospa_generator, "GOSPA Metrics") or _find_metric_by_title(
            gospa_generator, "GOSPA Metric"
        )
        gospa = _gospa_component_means(gospa_metric)

        siap_generator = _lookup_generator(metrics, "siap")
        siap = {title: _scalar_from_metric(_find_metric_by_title(siap_generator, title)) for title in _SIAP_TITLES}
