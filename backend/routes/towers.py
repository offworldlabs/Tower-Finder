...
async def find_towers(
    ...
    scorer = (
        (lambda ts: annotate_coverage_added(ts, lat, lon, geoms))
        if coverage and geoms
        else None
    )

    towers = await run_in_threadpool(
        process_and_rank, raw, lat, lon,
        limit=effective_limit, user_frequencies=user_freqs, radius_km=effective_radius,
