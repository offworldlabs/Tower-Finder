...
def _aircraft_at(
    ...
    return types.SimpleNamespace(lat=lat, lon=lon, object_id=object_id, speed_km_s=speed_km_s, heading_deg=heading_deg)
...
def test_perfect_tracks():
    ...
    """
    recorder = MetricRecorder(REF_LAT, REF_LON, duration_s=60.0, sample_dt_s=5.0, hold_max_age_s=12.0)
    ...
    t = 0.0
    while t <= 60.0:
        aircraft = [_aircraft_at(oid, e0, n0, hdg, spd, t) for oid, (e0, n0, hdg, spd) in tracks.items()]
...
def test_no_data_returns_none():
    ...
    recorder = MetricRecorder(REF_LAT, REF_LON, duration_s=30.0, sample_dt_s=5.0, hold_max_age_s=12.0)
...
def test_hold_window():
    ...
    def _run(stop_publishing_at):
        recorder = MetricRecorder(REF_LAT, REF_LON, duration_s=60.0, sample_dt_s=5.0, hold_max_age_s=hold_s)
        t = 0.0
        while t <= 60.0:
